// Cài đặt cơ sở dữ liệu bằng MySQL / MariaDB qua giao thức gốc.
#pragma once
...
public:
    explicit MysqlDatabase(DatabaseConfig config);
    ~MysqlDatabase() override;

    std::string kind() const override { return "mysql"; }
...
private:
    // Chạy truy vấn, tự kết nối lại một lần nếu mất kết nối.
    MysqlResult run(const std::string& sql, std::string& error);
    bool ensureConnection(std::string& error);
